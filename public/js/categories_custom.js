...
$(document).ready(function () {
	...
	*/
    ...
    function initPriceSlider()
    {
    	if($("#slider-range").length)
    	{
			...
			$( "#amount" ).val( "R$" + $( "#slider-range" ).slider( "values", 0 ) + " - $" + $( "#slider-range" ).slider( "values", 1 ) );
			$('.filter_price').on('mouseup', function()
			{
				$('.product_grid').isotope({
		            filter: function()
		            {
			        	...
			        	var itemPrice = Math.trunc($(this).find('.product_price').clone().children().remove().end().text().replace( 'R$', '' ).replace(',','.'));
  ...
  function initProductsHeight() {
    if ($(".sidebar_left").length) {
      var sidebarH = $(".sidebar_left").outerHeight(true);
  ...
  $(".btn_filter").click(() => {
    ...
    $("#sidebar_left").animate({ left: "-286px" });
    filter = false;
    return;
  });
  ...
  $(".close_filter").click(() => {
    if (!filter) {
      $("#sidebar_left").animate({ left: "0" }, "slow");
      filter = true;
      return;
    }

    $("#sidebar_left").animate({ left: "-286px" }, "slow");
    filter = false;
    return;
  });
});
